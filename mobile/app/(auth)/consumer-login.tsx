...
import { useConsumerLogin } from '@/features/auth/hooks/useConsumerAuth';
...
export default function ConsumerLoginScreen() {
  ...
  const { mutate, isPending, error } = useConsumerLogin();
    ...
    <SafeAreaView className="flex-1 bg-white">
      ...
      >
        <View className="mb-10">
          <Text className="text-3xl font-bold text-gray-900">Бонусна картка</Text>
          <Text className="text-base text-gray-500 mt-2">Увійдіть у свій акаунт покупця</Text>
        </View>
        ...
        <View className="gap-4">
          ...
          <TouchableOpacity onPress={() => router.replace('/(auth)/select-role')} className="items-center mt-1">
            <Text className="text-gray-400 text-sm">Я співробітник</Text>
          </TouchableOpacity>
        </View>
      </KeyboardAvoidingView>
    </SafeAreaView>
