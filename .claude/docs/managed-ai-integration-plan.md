...
| **4b-backend. Споживчий агент** ✅ TASK-709 | `ConsumerAssistantService` (`Application/Features/ConsumerAssistant/`, не `Infrastructure.AI`) — `ai_consumer`-конфіг через `ITenantSessionOverride`, споживчі дані з `ILoyaltyService`, guardrail `AiSlot.Consumer` через новий `IAiPromptResolver.Compose` + `IAiClientFactory.CreateOrEnv`; `POST /api/consumer/assistant/{tenantId}/ask` + rate-limit `consumer-ai` (6/3хв per consumer); `consumer_ai_requests` аудит-таблиця (метадані + 200-симв. excerpts) | середній, main session. Деталі: `managed-ai-phase4-plan.md` |
| **4b-mobile. Чат-екран** | у споживчому застосунку (`mobile/`), викликає `/api/consumer/assistant/{tenantId}/ask` | не почато — mobile-власнику (`mobile/` = територія Codex) |

Фаза 1 самодостатня: закриває головну вимогу (ізоляція + провайдер керує) на Claude-only,
без абстракції провайдерів.

**Пост-Фаза-3 фікс `03e6c9ed`:** пресет, збережений без ключа клієнта, більше не читається
як «не підключено» — `GetAsync` повертає рядок незалежно від наявності ключа, keyless-рядок
працює на env-ключі провайдера (`AiClientFactory`).
