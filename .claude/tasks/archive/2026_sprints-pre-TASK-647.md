# Архів задач (TASK-647 і старіші)

Винесено з `current.md` 2026-09-02, щоб активний файл лишався малим. Тут — хвіст
спринту v4.5 «Security Hardening» (TASK-329..647), усі попередні спринти (v4.4 → v2.1)
та історичний список Done. Роздільники `# Previous sprint — …` збережено всередині.
`grep` по TASK-ID для конкретної задачі; повна історія змін — у git.
